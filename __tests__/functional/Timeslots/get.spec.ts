...
describe('Timeslots functional tests', () => {
	...
	beforeEach(async () => {
		result1 = await populateServiceAndServiceProvider({
			nameService: NAME_SERVICE_1,
			serviceProviderName: SERVICE_PROVIDER_NAME_1,
		});
		result2 = await populateServiceAndServiceProvider({
			nameService: NAME_SERVICE_2,
			serviceProviderName: SERVICE_PROVIDER_NAME_2,
		});
		result3 = await populateServiceAndServiceProvider({
	...
	it('service admin should only get timeslot schedules for their service', async () => {
		await pgClient.mapServiceAdminToService({
			serviceId: result1.service.id,
			nameService: NAME_SERVICE_1,
	...
	it('citizen should not get any timeslot schedules', async () => {
		try {
			await CitizenRequestEndpointSG.create({ serviceId: result1.service.id }).get(
				`/service-providers/${serviceProviderId1}/timeslotSchedule`,
			);
			await CitizenRequestEndpointSG.create({ serviceId: result2.service.id }).get(
				`/service-providers/${serviceProviderId2}/timeslotSchedule`,
			);
			await CitizenRequestEndpointSG.create({ serviceId: result3.service.id }).get(
				`/service-providers/${serviceProviderId3}/timeslotSchedule`,
			);
		} catch (e) {
			expect(e.message).toBe(ERROR_MESSAGE);
