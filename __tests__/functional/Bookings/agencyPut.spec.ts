...
describe('Bookings functional tests as agency for PUT request', () => {
	...
	it('agency should be able to update a booking to a provided slot with accepted status', async () => {
	...
	it('agency should be able to update a booking to out of slot timeslot with accepted status', async () => {
		await createInSlotBooking();
		const getResponse = await AgencyRequestEndpointSG.create({
			serviceId: `${serviceId}`,
		}).get(`/bookings`, {});
		...
		const bookingId = getResponse.body.data[0].id;
		const putResponse = await AgencyRequestEndpointSG.create({
			// const putResponse = await OrganisationAdminRequestEndpointSG.create({
			serviceId: `${serviceId}`,
		}).put(`/bookings/${bookingId}`, {
			body: {
				startDateTime: new Date(Date.UTC(2051, 11, 10, 1, 0)),
