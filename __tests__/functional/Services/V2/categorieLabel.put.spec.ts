...
describe('Tests endpoint', () => {
	...
	beforeEach(async (done) => {
		...
		response = await OrganisationAdminRequestEndpointSG.create({}).post(
			'/services',
			{
		...
		serviceCreated = response.body.data as ServiceResponseV2;
	...
	it('Should update service with a new category', async () => {
		...
		response = await OrganisationAdminRequestEndpointSG.create({}).put(
			`/services/${serviceCreated.id}`,
			{
				body: { name: SERVICE_NAME, labels, categories: newCategories },
			},
			'V2',
		);
		const service = response.body.data as ServiceResponseV2;
		expect(response.statusCode).toEqual(200);
	...
	it('Update duplicate label if label move to no category', async () => {
		...
		let service = response.body.data as ServiceResponseV2;
		expect(response.statusCode).toEqual(200);
		...
		service = response.body.data as ServiceResponseV2;
		expect(response.statusCode).toEqual(200);
