...
export class LifeSGObserver implements IObserver {
	...
	/**
	 * This method will send an accepted or cancelled booking to LifeSG's appointment service. So a booking entity on BSG is an appointment entity in LifeSG
	 * Over at LifeSG, what they will do with the booking data is to display it in the LifeSG mobile app, so that LifeSG users can view bookings on the LifeSG mobile app
	 * The following function will check that booking.service.mqSubscriber contains LifeSG enum, hence if we want bookings to be made to LifeSG, we will need to set the service level setting mqSubscriber to {lifesg}
	 *
	 * **IMPORTANT NOTEs**
	 * 1. As of Nov 2021, only HDB-VC service requires their bookings to be piped to LifeSG, hence we did a quick fix to hardcode that all bookings made will be piped to LifeSG as a HDB-VC booking: `lifeSGMQService.send(...., AppointmentAgency.HDBVC_BSG)`
	 ...
	 *
	 * 2. When comes to rescheduling, things gets a little more complicated because BSG bookings might have pending status, but LifeSG side does not have such a status for their appointments
	 * Currently not an issue because we only have 1 use case, and that's HDB-VC, and HDB-VC is auto accept flow, so their bookings will never become pending
