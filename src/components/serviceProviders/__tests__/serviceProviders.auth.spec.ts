...
describe('Service providers Auth', () => {
	it('should not be able to create a serviceProvider for serviceProvider', () => {
		...
		const userGroup = new ServiceProviderAuthGroup(User.createAdminUser({molAdminId: '', userName: '', email: '', name: ''}), serviceProvider);
		const authVisitor = new ServiceProvidersActionAuthVisitor(ServiceProvider.create('new sp', 1), CrudAction.Create);
	...
	it('should be able to create a serviceProvider for service admin', () => {
		...
		const userGroup = new ServiceAdminAuthGroup(User.createAdminUser({molAdminId: '', userName: '', email: '', name: ''}), [service]);
		const authVisitor = new ServiceProvidersActionAuthVisitor(ServiceProvider.create('new sp', 1), CrudAction.Create);
	...
	it('should be able to create a serviceProvider for org admin', () => {
		...
		const userGroup = new OrganisationAdminAuthGroup(User.createAdminUser({molAdminId: '', userName: '', email: '', name: ''}), [organisation]);
		const authVisitor = new ServiceProvidersActionAuthVisitor(ServiceProvider.create('new sp', 1), CrudAction.Create);
	...
	it('should be able to update a serviceProvider for org admin', () => {
		const organisation = new Organisation();
		const userGroup = new OrganisationAdminAuthGroup(User.createAdminUser({molAdminId: '', userName: '', email: '', name: ''}), [organisation]);
		const authVisitor = new ServiceProvidersActionAuthVisitor(ServiceProvider.create('new sp', 1), CrudAction.Update);
		authVisitor.visitOrganisationAdmin(userGroup);
