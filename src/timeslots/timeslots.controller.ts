...
export class TimeslotsController extends Controller {
	...
	@Get("availability")
	@Security("service")
	public async getAggregatedTimeslots(@Query() startDate: Date, @Query() endDate: Date, @Header('x-api-service') _?): Promise<TimeslotResponse[]> {
		...
		return await this.timeslotsService.getAggregatedTimeslots(startDate, endDate);
	}
}
